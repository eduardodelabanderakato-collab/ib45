...
const seed={ plans, subjects:JSON.parse(fs.readFileSync(path.join(D,'subjects.json'))), assessments:JSON.parse(fs.readFileSync(path.join(D,'assessments.json'))), tour:JSON.parse(fs.readFileSync(path.join(D,'tour.json'))), stanford:JSON.parse(fs.readFileSync(path.join(D,'stanford.json'))), briefs:JSON.parse(fs.readFileSync(path.join(__dirname,'..','data','briefs.json'))), focus:JSON.parse(fs.readFileSync(path.join(__dirname,'..','data','state.json'))).focus||null, leisure:JSON.parse(fs.readFileSync(path.join(__dirname,'..','data','state.json'))).leisure||[] };
...
let html=fs.readFileSync(path.join(__dirname,'template.html'),'utf8').replace('/*__SEED__*/{}', JSON.stringify(seed)).split('__TITLE__').join(title).replace('avatar: null', 'avatar: '+JSON.stringify(avatar)).replace('holo: null', 'holo: '+JSON.stringify(holo)).replace('__BUILT__', new Date().toISOString());
