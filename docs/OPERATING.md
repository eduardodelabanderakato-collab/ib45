...
**The app ('Life'):** https://eduardodelabanderakato-collab.github.io/ib45/ (GitHub Pages; built by `src/build.js` from data/state.json every push and daily 05:30; ticks/reports/errors/contract live in the browser's localStorage under `life-db`). The Claude Artifact copy at https://claude.ai/artifact/1yfwsi3bMVerDih52BtTSD still exists but he does not want it — source `app/template.html`, build `node app/build-app.js "<name>"` → `app/index.html`, republish with the Artifact tool (same path keeps the URL). Data lives in the artifact DB (ArtifactData tool): `state/{subjects,assessments,tour,stanford}`, `plan/<date>` (one per day, from `app/seed/plan/*.json`), `days/<date>` (his ticks/starts/report), `audit/<date>` (nightly, from Focus Flight), `errors/<id>`, `stanford/progress`, `contract/main`. Weekly: regenerate seeds (`node -e` script in git history / rerun the seed step), batch-set `plan/*` for the next 14 days.
