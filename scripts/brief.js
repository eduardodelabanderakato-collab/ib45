...
const FACE = { phys: ['Isaac Newton', 'Newton, who wrote the rules you are being tested on'], chem: ['Dmitri Mendeleev', 'Mendeleev, who put the table in order'], math: ['Leonhard Euler', 'Euler, who did more math than anyone'], econ: ['Adam Smith', 'Adam Smith, before the diagrams'], eng: ['Marjane Satrapi', 'Marjane Satrapi, author of Persepolis'] };
...
const C = { ink: '#0e1a33', body: '#2b3446', dim: '#6f7a90', line: '#e3e8f0', bg: '#ffffff', card: '#ffffff', acc: '#0b7fb0', hi: '#ffe58f', warm: '#f5a623', box: '#fff8e1', navy: '#061633' };
...
<tr><td align="center" style="padding:0 0 18px;${font};font-size:12px;letter-spacing:2px;color:${C.dim};text-transform:uppercase;line-height:1.5">${esc(wd)}, ${d0.getUTCDate()} ${MONTHS[d0.getUTCMonth()].charAt(0) + MONTHS[d0.getUTCMonth()].slice(1).toLowerCase()} ${d0.getUTCFullYear()} &nbsp;•&nbsp; reading time: ${readMin} minute${readMin === 1 ? '' : 's'}</td></tr>
...
${stanford.length ? dots + label('stanford · this week') + P_(stanford.map(m => `<b>${P.pretty(m.date)}</b> &nbsp;${esc(m.title)}`).join('<br>')) : ''}
...
${NEWS.ai || (NEWS.economist && NEWS.economist.length) ? dots + label('two headlines') + P_([NEWS.ai ? `🤖 <a href="${NEWS.ai.url}" style="color:${C.ink};text-decoration:none;font-weight:700">${esc(NEWS.ai.title)}</a> <span style="color:${C.dim};font-size:14px">· ${esc(NEWS.ai.source)}</span>` : '', NEWS.economist && NEWS.economist[0] ? `📈 <a href="${NEWS.economist[0].url}" style="color:${C.ink};text-decoration:none;font-weight:700">${esc(NEWS.economist[0].title)}</a> <span style="color:${C.dim};font-size:14px">· The Economist, one example card for Paper 1</span>` : ''].filter(Boolean).join('<br>')) : ''}
${dots}
${label('read tonight')}
...
<tr><td style="background:#f4f4f4;padding:20px 18px 34px;${font};font-size:13px;line-height:1.6;color:${C.dim};text-align:center">Life · edition ${editionNo} · sent every morning at 06:45 from your own plan.<br>Tests, holidays, changes: tell Claude in the IB45 chat.</td></tr>
