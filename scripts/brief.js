...
const link = (id) => LINKS[id] ? `<a href="${LINKS[id][1]}" style="color:#0b5fff;text-decoration:none;font-weight:700">${esc(LINKS[id][0])}</a>` : '';
...
${P_(`<b>Why it matters.</b> ${near[0] && near[0].daysLeft <= 3 ? `The ${esc(near[0].subjectShort)} test is ${near[0].daysLeft === 0 ? 'today' : near[0].daysLeft === 1 ? 'tomorrow' : 'in ' + near[0].daysLeft + ' days'}; today's blocks are the last edits before it.` : 'Pre-learning today is what makes tomorrow's classes a review instead of a first look.'} Sprint 1 is what counts as a day started.`)}
