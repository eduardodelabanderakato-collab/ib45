...
const POOL = require('./photo-pools.js'); const FACE = Object.fromEntries(['phys', 'chem', 'math', 'econ', 'eng', 'port'].map(k => [k, POOL.pick(k, day)])); const STANFORD_PHOTO = POOL.pick('stanford', day); const AI_PHOTO = POOL.pick('ai', day);
...
${stanford.length ? dots + label('stanford · this week') + thumb(STANFORD_PHOTO, stanford.map(m => `<b>${P.pretty(m.date)}</b> &nbsp;${esc(m.title)}`).join('<br>')) : ''}
...
${NEWS.ai || (NEWS.economist && NEWS.economist.length) ? dots + label('two headlines') + thumb(AI_PHOTO, [NEWS.ai ? `🤖 <a href="${NEWS.ai.url}" style="color:${C.ink};text-decoration:none;font-weight:700">${esc(NEWS.ai.title)}</a> <span style="color:${C.dim};font-size:14px">· ${esc(NEWS.ai.source)}</span>` : '', NEWS.economist && NEWS.economist[0] ? `📈 <a href="${NEWS.economist[0].url}" style="color:${C.ink};text-decoration:none;font-weight:700">${esc(NEWS.economist[0].title)}</a> <span style="color:${C.dim};font-size:14px">· The Economist, one example card for Paper 1</span>` : ''].filter(Boolean).join('<br>')) : ''}
${dots}
${label('read tonight')}
