...
<tr><td style="background:#8C1515;background-image:linear-gradient(90deg,#8C1515 0%,#C8452F 55%,#F5A623 100%);padding:34px 28px;text-align:center"><div style="font-size:40px;font-weight:800;color:#fff;letter-spacing:-.03em;line-height:1">✈︎ flight deck</div><div style="margin-top:8px;font-size:11px;letter-spacing:.14em;color:rgba(255,255,255,.85);text-transform:uppercase;font-weight:600">Eduardo · IB 45 · Class of 2032</div></td></tr>
<tr><td style="padding:26px 28px 0">
  ...
  ${stanford.length ? dots + label('stanford · class of 2032') + H(`${P.addDays(day, 0) <= '2027-11-01' ? Math.round((new Date('2027-11-01T12:00:00Z') - new Date(day + 'T12:00:00Z')) / 86400000) : 0} days to Restrictive Early Action`) + `<div style="font-size:15px;line-height:1.8">${stanford.map(m => `<b style="${mono};font-size:13px">${m.date.slice(5)}</b> &nbsp;${esc(m.title)} <span style="color:${C.dim};font-size:13px">· ${esc(m.owner)}</span><br>`).join('')}</div>` : ''}
