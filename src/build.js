...
async function main() {
  ...
  try { const holo = fs.existsSync(path.join(ROOT, 'app/assets/hologram-figure.png')) ? 'data:image/png;base64,' + fs.readFileSync(path.join(ROOT, 'app/assets/hologram-figure.png')).toString('base64') : '';
    const bp = await browser.newPage({ viewport: { width: 1200, height: 420 }, deviceScaleFactor: 1 });
    await bp.setContent(`<!doctype html><html><head><meta charset="utf-8"><link href="https://fonts.googleapis.com/css2?family=Inter+Tight:wght@700;800&display=block" rel="stylesheet"><style>html,body{margin:0;width:1200px;height:420px;overflow:hidden}body{background:radial-gradient(900px 500px at 50% 20%,#0e3a7a 0%,#061633 55%,#03091c 100%);font-family:'Inter Tight',Inter,-apple-system,sans-serif;color:#EAF2FF;position:relative}.bits{position:absolute;inset:0;font:600 16px Menlo,monospace;color:rgba(120,200,255,.35)}.bits span{position:absolute}.w{position:absolute;left:90px;top:120px}.w h1{margin:0;font-size:132px;font-weight:800;letter-spacing:-.05em;line-height:.9}.w p{margin:14px 0 0 6px;font-size:18px;letter-spacing:.22em;text-transform:uppercase;color:#7fd8ff;font-weight:700}.h{position:absolute;right:120px;top:10px;height:400px;filter:drop-shadow(0 0 30px rgba(95,212,255,.6))}.beam{position:absolute;right:60px;bottom:-40px;width:340px;height:140px;background:radial-gradient(closest-side,rgba(95,212,255,.5),rgba(95,212,255,0))}</style></head><body><div class="bits">${Array.from({length:60},()=>`<span style="left:${Math.random()*1200}px;top:${Math.random()*420}px;opacity:${(0.2+Math.random()*0.6).toFixed(2)}">${Math.random()<.5?'0':'1'}</span>`).join('')}</div><div class="beam"></div>${holo?`<img class="h" src="${holo}">`:''}<div class="w"><h1>Life</h1><p>Eduardo · IB 45 · Class of 2032</p></div></body></html>`, { waitUntil: 'networkidle' });
