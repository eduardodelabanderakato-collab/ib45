...
async function main() {
  ...
  try { const { execFileSync } = require('child_process'); const F = require('./flights.js'); const t = state.tour || { at: 'HND' }; const it = F.itinerary(t.at || 'HND', plan.blocks.filter(b => ['sprint', 'retrieval'].includes(b.kind)), t.visited);
  ...
  try { const { execFileSync } = require('child_process'); const F = require('./flights.js'); let at = (state.tour && state.tour.at) || 'HND'; const visitedSim = [...((state.tour && state.tour.visited) || [])]; const seedDir = path.join(ROOT, 'app/seed/plan'); fs.mkdirSync(seedDir, { recursive: true }); for (const f of fs.readdirSync(seedDir)) fs.unlinkSync(path.join(seedDir, f));
    for (let d = iso, i = 0; i < 14; d = P.addDays(d, 1), i++) { const p = P.planFor(state, d); const fly = p.blocks.filter(b => ['sprint', 'retrieval'].includes(b.kind)); const it = F.itinerary(at, fly, visitedSim); visitedSim.push(...it.legs.filter(l => l.flight).map(l => l.flight.to)); const by = Object.fromEntries(it.legs.map(l => [l.start, l.flight]));
