...
function defaultTask(state, iso, block, cds) {
  ...
  if (block.slot === 'SAT1' && isoWeek(iso) % 2 === 0 && !live.length) return iso <= '2026-11-07' ? { subject: 'sat', task: 'Full SAT practice test, part 1: Reading & Writing modules 1–2 (Bluebook, 64 min) · no phone, exam conditions' } : { subject: HL_ROTATION[isoWeek(iso) % 3 + 1] || 'math', task: 'Full timed IB past paper under exam conditions (Paper 1), then mark it with the scheme' };
  if (block.slot === 'SAT2' && isoWeek(iso) % 2 === 0 && !live.length) return iso <= '2026-11-07' ? { subject: 'sat', task: 'Full SAT practice test, part 2: Math modules 3–4 (70 min) · score it · every miss into the error log' } : { subject: 'all', task: 'Mark the paper: every lost mark classified (concept / method / slip / misread / time) · plan the fixes' };
