...
function defaultTask(state, iso, block, cds) {
  ...
  if (block.slot === 'S3') { if (iso <= '2026-11-07' && wd === 5) return { subject: 'sat', task: 'SAT Math: 22 questions timed (35 min, Bluebook or SAT Mastery) · review every miss · log the error type' }; if (iso <= '2026-11-07' && wd === 2 && !live.length) return { subject: 'sat', task: 'SAT Reading & Writing: one timed module (32 min) · review every miss · 10 min on the weakest question type' }; const id = wd === 2 ? 'eng' : 'port'; const s = state.subjects[id]; return { subject: id, task: s.next[0] }; }
  const satSat = daysBetween('2026-09-26', iso) % 14 === 0 && iso >= '2026-09-26';
  if (block.slot === 'SAT1' && satSat) return iso <= '2026-11-07' ? { subject: 'sat', task: 'Full SAT practice test, part 1: Reading & Writing modules 1–2 (Bluebook, 64 min) · no phone, exam conditions' } : { subject: HL_ROTATION[isoWeek(iso) % 3 + 1] || 'math', task: 'Full timed IB past paper under exam conditions (Paper 1), then mark it with the scheme' };
  if (block.slot === 'SAT2' && satSat) return iso <= '2026-11-07' ? { subject: 'sat', task: 'Full SAT practice test, part 2: Math modules 3–4 (70 min) · score it · every miss into the error log' } : { subject: 'all', task: 'Mark the paper: every lost mark classified (concept / method / slip / misread / time) · plan the fixes' };
